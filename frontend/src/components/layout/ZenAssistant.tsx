...
export default function ZenAssistant({
  ...
  const predefinedPrompts = [
    ...
    "Trang phục nào thoáng mát, thoải mái nhất?",
    "Chất liệu vải đũi và lụa organic khác nhau thế nào?",
  ];
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    setConversation((prev: ChatMessage[]) => [...prev, newUserMsg]);
    setUserInput("");
    setIsLoading(true);

    try {
      // Fetch response from server secure endpoint
      ...
      const newBotMsg: ChatMessage = {
        sender: "assistant",
        text: data.text || "Từ Tâm Phục có thể giúp gì thêm cho quý khách không ạ?",
        timestamp: new Date(),
      };

      setConversation((prev: ChatMessage[]) => [...prev, newBotMsg]);
    } catch (err) {
      console.error("Consultation Error:", err);
      // Fallback response when server is not accessible or mock mode
      const newBotMsg: ChatMessage = {
        sender: "assistant",
        text: "Hệ thống đang bận một chút. Quý khách vui lòng thử lại sau vài giây hoặc liên hệ trực tiếp với chúng tôi nhé.",
        timestamp: new Date(),
      };
      setConversation((prev: ChatMessage[]) => [...prev, newBotMsg]);
    } finally {
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        ...
        >
          ...
          <div className="px-5 py-6 bg-brand-ivory border-b border-brand-sand/40 flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="text-brand-secondary hover:text-brand-primary cursor-pointer p-1 rounded-full hover:bg-brand-sand/40 transition-colors bg-transparent border-none"
            >
              <X size={20} />
            </button>
          </div>
          ...
          {selectedProduct && (
            <div className="bg-brand-sand/30 border-b border-brand-sand/50 px-5 py-3 flex gap-3 items-center text-[11px]">
              ...
              <span className="text-brand-primary font-semibold">
                Đang xem sản phẩm: <em className="underline not-italic">"{selectedProduct.name}"</em>
              </span>
            </div>
          )}
            ...
            {conversation.length < 3 && !isLoading && (
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] font-bold text-brand-secondary/70 uppercase tracking-widest flex items-center gap-1">
                  <HelpCircle size={10} /> Câu hỏi gợi ý:
                </span>
              ...
              <button
                ...
                className={`px-4 bg-brand-primary text-brand-bg rounded-sm flex items-center justify-center transition-all shadow-md ${
                  isLoading || !userInput.trim()
                    ...
                    : "hover:bg-brand-brown active:scale-95 cursor-pointer border-none"
                }`}
                id="assistant-submit"
              >
                <Send size={15} />
